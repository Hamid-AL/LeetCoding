...
> 2. Click on **Go to Azure OpenAI Studio** in the top left corner.
> 3. In the **Deployments** section, view the models that are currently deployed.
>> **Note:**  
>> If no model is deployed, deploy one from this section. Make sure the deployment name matches the name specified in the `deployment` field in your configuration file (`config.yaml`).
>> The models specified may not perform as expected in your environment, and the app may throw errors if the selected models are incompatible or insufficient for your use case. If you encounter issues, consider switching to a larger model to improve compatibility and performance.


> [!TIP]
> ### Document Intelligence
> Create a Document Intelligence resource, retrieve the access key and endpoint, and enter them in the `config.yaml` file.
> 
> **Steps**
> 
> 1. **Log in to Azure Portal**: Go to [Azure Portal](https://portal.azure.com) and sign in to your Azure account.
> 
> 2. **Create Resource**:
>    - Click on **Create a resource**.
>    - Search for **Document Intelligence** (also known as **Azure Form Recognizer**).
>    - Select **Document Intelligence** from the search results, then click **Create**.
> 
...
> [!TIP]
> ### Configuring Local Settings for Azure Integration
> Rename `local.example.settings.json` to `local.settings.json` and populate it with the correct information:
